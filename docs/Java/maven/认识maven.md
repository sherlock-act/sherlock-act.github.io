...
- 如果重新下载maven的话需要到[maven下载页面](http://maven.apache.org/download.cgi)进行下载
- 下载完成后直接解压即可
- 然后需要执行`mvn help:system`，如果提示命令错误需要自行添加环境变量
- 等待命令执行完成后就会生成`C:\Users\用户名\.m2`目录，目录下有`repository`目录与`settings.xml`文件夹
- 如果没有`settings.xml`文件可以自行从网上下载 一个，下面提供一个阿里镜像的`settings.xml`文件
